...
router.post('/riders', requireRole('admin', 'dispatcher'), async (req, res) => {
  try {
    ...
    const prefix = (org.slug || 'RWK').substring(0, 3).toUpperCase();
...
router.get('/', requireRole('admin', 'dispatcher'), async (req, res) => {
  try {
    ...
    if (date) {
      ...
      query.tripDate = {};
      if (dateFrom) { const s = new Date(dateFrom); s.setHours(0,0,0,0); query.tripDate.$gte = s; }
      if (dateTo)   { const e = new Date(dateTo);   e.setHours(23,59,59,999); query.tripDate.$lte = e; }
    }
...
router.get('/driver/my-trips', async (req, res) => {
  try {
    ...
    const rawTrips = await Trip.find(query)
      .populate('vehicle', 'name licensePlate color')
    ...
    const trips = rawTrips.map(trip => {
      ...
      // Build a map: riderId -> { pickup stop, dropoff stop }
      const riderStops = {};
      t.stops.forEach(stop => {
        const key = stop.riderId?._id?.toString() || stop._id.toString();
        if (!riderStops[key]) riderStops[key] = { pickup: null, dropoff: null, riderId: stop.riderId };
        if (stop.type === 'pickup')  riderStops[key].pickup  = stop;
        if (stop.type === 'dropoff') riderStops[key].dropoff = stop;
      });
      // Rebuild stops with combined fields for driver app
      t.stops = t.stops.map(stop => ({
        ...stop,
        pickupAddress:       stop.type === 'pickup'  ? stop.address : (riderStops[stop.riderId?._id?.toString() || stop._id.toString()]?.pickup?.address || stop.address),
        dropoffAddress:      stop.type === 'dropoff' ? stop.address : (riderStops[stop.riderId?._id?.toString() || stop._id.toString()]?.dropoff?.address || ''),
        scheduledPickupTime: stop.scheduledTime || null,
      }));
      return t;
    });
...
router.post('/optimize-preview', requireRole('admin', 'dispatcher'), async (req, res) => {
  try {
    const { stops, homeBase, tripDate } = req.body;
    ...
    const result = await optimizeRoute({
      homeBase: homeBase || { address: '1523 16th St S, St. Petersburg, FL 33705', lat: 27.7531, lng: -82.6652 },
      stops,
...
router.post('/:id/optimize', requireRole('admin', 'dispatcher'), async (req, res) => {
  try {
    ...
    const { optimizeRoute } = require('../services/routeOptimizer');
    const result = await optimizeRoute({
      homeBase: trip.homeBase || { address: '1523 16th St S, St. Petersburg, FL 33705', lat: 27.7531, lng: -82.6652 },
      stops: trip.stops,
